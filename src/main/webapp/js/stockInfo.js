...
function convertApiDataToDisplayFormat(apiData) {
  ...
  const previousClose = parseInt(apiData.previousClose || apiData.mkp);
  // 전일 종가가 없으면 시가 사용
...
function selectStock(stockName) {
  ...
  if (stock) {
    fetchStockInfo(stock.name);
    const displayData = convertApiDataToDisplayFormat(stock);
...
function updateStockInfo(stockData) {
  ...
  const changeText = `${change > 0 ? "+" : ""}${formatNumber(change)} (${
    change > 0 ? "+" : ""
  }${changePercent}%)`;
...
function updateStockInfoFromApi(apiData) {
  const displayData = convertApiDataToDisplayFormat(apiData);
  updateStockInfo(displayData);
}
